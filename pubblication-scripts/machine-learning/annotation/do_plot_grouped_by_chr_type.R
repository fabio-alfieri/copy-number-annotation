do_plot_grouped_by_chr_type <- function(input_df, tot_chrom_bins_per_type, col.to.group, distinct_colors, save = TRUE){
  ...
  if (save) {
    ...
    if ("top1" %in% col.to.group) {
      filename <- paste0(i, "_annot_grouped_by_chr_type_detailed.pdf")
    } else {
      filename <- paste0(i, "_", model_class, "_", pval_thr, "_", diploidy_threshold, "_", ns_threshold, "_grouped_by_chr_tt.pdf")
    }
