...
X_data <- ml_table[,!colnames(ml_table) %in% c('bin','Type','ampl_score', 'del_score', 'train', 'test')]
features <- colnames(X_data)
y_data <- ml_table[,c("train", "test", label)]

dftrain <- ml_table[ml_table$train == T,]
dftest <- ml_table[ml_table$test == T,]

ml_table$train <- NULL; ml_table$test <- NULL
...
save(Output.regressor, 
     file = paste0("/home/ieo7429/Scrivania/results_regressor_gab/",
                   folder,"/",
                   "Output.regressor_PRO_",
                   train_length, "_",
                   test_length, "_",
                   num_of_skipped_chrs, "_",
                   clus.group,"_",
                   classS,"_",
                   ampl_or_del,"_",
                   level,"_",
