...
gene.lists.per.bin <- function(bin){
  gene.set <- list("ppis.trans" = as.character(strsplit(backbone[backbone$bin == bin, "PPIs_063.trans-HIPPIE"],"::")[[1]]), #PPIs in trans
                   "all.int.trans" = union(as.character(strsplit(backbone[backbone$bin == bin, "Partners.trans"],"::")[[1]]),
                                           as.character(strsplit(backbone[backbone$bin == bin, "PPIs_063.trans-HIPPIE"],"::")[[1]])) #All interactions
...
selected.columns <- c("bin","chr","start_bin","end_bin",
                      "genes","n_genes","Partners.trans","PPIs_063.trans-HIPPIE")
...
save(Results, file = "./Data/GTEx_rna_protein_level_differentdatasets_per_bins_HIPPIE_cutoff_063.RData")
