...
if(length(unique(chromosomes_pair)) > 1){
  chromosome_idxs_train <- which(chr_nums == chromosomes_pair[1])
  chromosome_idxs_test <- which(chr_nums == chromosomes_pair[2])
} else if (length(unique(chromosomes_pair)) == 1){
  chromosome_idxs <- which(chr_nums == chromosomes_pair[1])
  X_data <- X_data[chromosome_idxs,]
  y_data <- y_data[chromosome_idxs]
  set.seed(122)
  chromosome_idxs_train <- createDataPartition(y_data, p = 0.8, list = FALSE)
  chromosome_idxs_test <- -chromosome_idxs_train
}

# LOCO split

X_train <- X_data[chromosome_idxs_train, ]
X_test <- X_data[chromosome_idxs_test, ]
y_train <- y_data[chromosome_idxs_train]
y_test <- y_data[chromosome_idxs_test]
...
saveRDS(xgb_tune, file = paste0('/home/ieo7429/Scrivania/hyperparam_tuning/', classS, '_hyper_tuning_CSP_scaled_', chromosomes_label, '.rds')) # remember here it's related to last trained model. Improve
