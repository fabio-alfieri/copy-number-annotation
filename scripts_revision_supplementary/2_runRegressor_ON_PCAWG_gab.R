...
for(j in 1:nrounds){
  ...
  # For the scope of the project (i.e. building a genome-wide annotation), we need all data.
  # This means that the SHAP matrix will be built on the whole dataset (Train + Test)
  ...
  # # Plot the performance monitoring
  # rmse.plot <- ggplot(evaluation, aes(x = iter)) +
  #   geom_line(aes(y = train_rmse, color = "Train RMSE")) +
  #   geom_line(aes(y = test_rmse, color = "Test RMSE")) +
  #   labs(title = "Training vs Test RMSE", x = "Iteration", y = "RMSE", color = "") +
  #   theme_bw() +
  #   theme(legend.position = c(0.6,0.5)) +
  #   scale_color_manual(values = c("#e76f51","#03045e"))
  ...
  #### GAB EDITING HERE ADDING R2 and RMSE metrics ####
  ...
  spearman_corr_train <- cor(save.data_train[,label], save.data_train$prediction, method = "spearman")
  pearson_corr_train  <- cor(save.data_train[,label], save.data_train$prediction, method = "pearson")
  r2_train            <- caret::R2(save.data_train$prediction, save.data_train[,label])
  rmse_train          <- caret::RMSE(save.data_train$prediction, save.data_train[,label])
  
  
  # # Correlation plot
  # cor.plot<- ggplot(save.data, aes_string(x = label, y = "prediction")) +
  #   geom_point(color = "gray", alpha = 0.7) +
  #   geom_smooth(method = "lm", color = "#344e41", se = FALSE) +
  #   annotate("text", 
  #            x = min(save.data[,label]), 
  #            y = max(save.data[,"prediction"]), 
  #            label = paste0("Pearson: ", round(pearson_corr, 3), 
  ...
  #   theme_bw()
  # 
  # plots[[label]][[j]] <- ggarrange(rmse.plot, cor.plot)
  
  
  
  # Feature Importance using predict (SHAP values)
  ...
  Output.regressor.sub <- list("Model" = xgb_model,
                               "Performance" = list("Spearman" = spearman_corr, "Pearson" = pearson_corr, "R2" = r2, "RMSE" = rmse),
                               "Performance_train" = list("Spearman" = spearman_corr_train, "Pearson" = pearson_corr_train, "R2" = r2_train, "RMSE" = rmse_train),
                               "X_train" = dftrain,
                               "X_test" = dftest,
                               "y_train" = y_train,
                               "y_test" = y_test, 
...
save(Output.regressor, 
     file = paste0("/home/ieo7429/Scrivania/results_regressor_gab/",
                   folder,"/",
                   "Output.regressor_ON_PCAWG_",
                   clus.group,"_",
                   classS,"_",
                   ampl_or_del,"_",
                   level,"_",
