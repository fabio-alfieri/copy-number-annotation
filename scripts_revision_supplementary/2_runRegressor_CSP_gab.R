...
if(length(unique(chromosomes_pair)) > 1){
  dftrain <- ml_table[chromosome_idxs_train,]
  dftest <- ml_table[chromosome_idxs_test,]
} else if (length(unique(chromosomes_pair)) == 1){
  dftrain <- ml_table[chromosome_idxs,][chromosome_idxs_train,]
  dftest <- ml_table[chromosome_idxs,][chromosome_idxs_test,]
}


# 2. Convert data to XGBoost format
xgb_train_all = xgb.DMatrix(data = data.matrix(dftrain[,features]), label = dftrain[,label],nthread = 6)
xgb_test_all = xgb.DMatrix(data = data.matrix(dftest[,features]), label = dftest[,label],nthread = 6)
...
for(j in 1:nrounds){
  ...
  Output.regressor[[label]][[chromosomes_label]][[j]] <- Output.regressor.sub
}
