...
if(length(unique(cts_pair)) > 1){
  dftrain <- ml_table[ct_idxs_train,]
  dftest <- ml_table[ct_idxs_test,]
} else if (length(unique(cts_pair)) == 1){
  dftrain <- ml_table[ct_idxs,][ct_idxs_train,]
  dftest <- ml_table[ct_idxs,][ct_idxs_test,]
}
...
for(j in 1:nrounds){
  ...
  Output.regressor[[label]][[cts_label]][[j]] <- Output.regressor.sub
}
