...
model_class <- 'Chromosome-level' # Chromosome-level
...
backbone_annotated <- backbone.100kb %>%
  ...
  mutate(
    arm = case_when(
      end.x < start.y ~ "",              # before centromere start
      start.x > end.y ~ ""              # after centromere end
    )
...
score.df$type <- rep(names(combined_tables),each = 22)
...
tcga <- tcga[grep('Chrom',tcga$X),]
tcga$arm <- str_replace_all(tcga$X, 'Chrom','chr')
...
load(file = '/home/ieo7429/Scrivania/dev/Data/cell line data/cellLine_ratios/chr_ratio_gain_day28_hMEC.RData')
day28 <- freq_ratio_gain_tidy; rm(freq_ratio_gain_tidy)
load(file = '/home/ieo7429/Scrivania/dev/Data/cell line data/cellLine_ratios/chr_ratio_gain_day2_hMEC.RData')
...
save(cellLines.ratio, file = 'cellLine_ratios/cellLine.ratio_Chr_hMEC_gain.RData')
...
ggplot(cellLines.ratio, aes(x = ratio, y = `Negative Selection`)) +
  ...
  geom_smooth(method = 'lm') +
  geom_text(aes(label = arm))
ggplot(cellLines.ratio, aes(x = ratio, y = Occurrence+`Negative Selection`)) +
  geom_point() +
  facet_wrap(~day+comparison) +
  geom_smooth(method = 'lm') +
  geom_text(aes(label = arm))
