...
# GTEx mRNA and protein levels for PPIs from HIPPIE
# Date: August 8, 2024
...
gene.lists.per.bin <- function(bin){
  gene.set <- list("ppis.trans" = as.character(strsplit(backbone[backbone$bin == bin, "PPIs.trans-HIPPIE"],"::")[[1]]), #PPIs in trans
                   "all.int.trans" = union(as.character(strsplit(backbone[backbone$bin == bin, "Partners.trans"],"::")[[1]]),
                                           as.character(strsplit(backbone[backbone$bin == bin, "PPIs.trans-HIPPIE"],"::")[[1]])) #All interactions
...
rm(list = setdiff(ls(),c("backbone_tables_w_features",
                         "datasets",
                         "common.tissues",
                         "levels",
                         "selected.columns")))
...
gene.lists.per.bin <- function(bin){
  gene.set <- list("ppis.trans" = intersect(tissue.genes,as.character(strsplit(backbone[backbone$bin == bin, "PPIs.trans-HIPPIE"],"::")[[1]])), #PPIs in trans
