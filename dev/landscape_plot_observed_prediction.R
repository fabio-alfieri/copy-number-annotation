landscape_plot_observed_prediction <- function(filtered_landscape,
  ...
  unique_classes <- unique(as.character(filtered_landscape[[annot_to_plot]]))
  ...
  base_plot <- base_plot +
    ...
    labs(title = title, subtitle = subtitle, x = "Genomic Position", y = paste0("SCNA frequency (", cluster_mask, ")")) +
    theme_classic() + 
  ...
  if (!isFALSE(annot_to_plot_meta)) {
    base_plot <- add_segment_gradient_layer(base_plot, filtered_landscape, annot_to_plot_meta, "pred",
                                            "labels", unique_classes, ticksize, lower_limit, upper_limit,
                                            available_height = 0.3, mid_point = 0)
  }
